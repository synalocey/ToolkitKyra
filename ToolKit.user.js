...
uppercase;letter-spacing:.45px;margin-bottom:7px}.upload-manual{margin-top:12px;padding-top:12px;border-top:1px solid #dbe5ee}.upload-manual summary{color:#334155;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.45px;cursor:pointer}.upload-manual-content{padding-top:10px}.upload-box input{display:block;width:100%;font-size:11px;color:#475569}.upload-box input::file-selector-button{border:0;border-radius:8px;background:
...
<header class="hero">
  <div class="hero-inner">
    ...
    <div class="hero-source"><small>Current data source</small><strong id="heroSource">Checking local cache…</strong></div>
  </div>
</header>
...
<section id="overviewTab" class="tab-page active" role="tabpanel" aria-labelledby="overviewBtn"><div class="layout">
  <aside class="side">
    ...
    <div class="upload-box">
      <button id="sapRangeOpen" class="btn blue wide" type="button">Load / Refresh NSR</button>
      <div id="sapRangeDefault" class="sap-range-default"></div>
      <details class="upload-manual">
        <summary>Upload raw data</summary>
        <div class="upload-manual-content">
          <label for="rawFile">Choose CSV or Excel file</label>
          <input id="rawFile" type="file" accept=".xlsx,.xls,.csv">
        </div>
      </details>
      <div id="sourceStatus" class="source-status loading">Checking local NSR cache…</div>
    </div>
...
<div id="sapRangeModal" class="wf-modal" hidden>
  ...
  <section class="wf-box sap-range-box" role="dialog" aria-modal="true" aria-label="Load or refresh NSR date range">
    <form id="sapRangeForm" class="sap-range-form">
      ...
      <div class="wf-actions">
        ...
        <button id="sapRangeLoad" class="btn blue" type="submit">Load</button>
      </div>
    </form>
  </section>
</div>
...
:12px;font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bridge-download{display:inline-flex;align-items:center;margin-top:9px;text-decoration:none}.bobj-status-head{display:flex;align-items:center;justify-content:space-between;gap:8px}.bobj-status-actions{display:flex;align-items:center;gap:5px}.bobj-update{flex:none;padding:4px 8px}.bobj-data-source{font-variant-numeric:tabular-nums}.bobj-refresh-state{display:block;margin-top:4px;white-space:normal}.bobj-refresh-state.running{color:#92400e}.bobj-refresh-state.error{color:#b91c1c}.bridge-field{display:grid;gap:6px;margin-top:11px}.bridge-field label{color:#475569;font-size:10px;font-weight:600;
  ...
  <section class="bridge-card connection-card">
    ...
    <div class="bridge-status">
      <div class="bridge-status-item">
        ...
        <strong id="bridgeService">Checking 127.0.0.1:8765…</strong>
        <a id="bridgeDownload" class="bridge-btn light small bridge-download" href="${BRIDGE_DOWNLOAD_URL}" download="SAPGUIcontrol.exe" target="_blank" rel="noopener noreferrer" title="Download SAPGUIcontrol if the local Bridge is not installed." hidden>Download Bridge</a>
      </div>
...
if(currentPath===APP_PATH){
  ...
  whenBody(mountBridgeApp);
}else if(unsafeWindow.location.hostname.toLowerCase()==='fep.lamresearch.com')addNsrHeaderButton();

function whenBody(callback){
  if(document.body)callback();else document.addEventListener('DOMContentLoaded',callback,{once:true});
}

async function addNsrHeaderButton(){
  while(true){
    const target=document.getElementById('shellAppTitle-button'),title=target&&target.querySelector('.sapUshellAppTitleText');
...
function mountApp(){
  ...
  const title=document.createElement('title');title.textContent='NSR Control Tower';
...
async function refreshBridge(){
  setBridgeBusy(true);setBridgeBadge('Checking','');bridgeId('bridgeService').textContent='Checking 127.0.0.1:8765…';bridgeId('bridgeDownload').hidden=true;
  try{
    ...
    bridgeId('bridgeService').textContent=error.status?'Local bridge responded with an error':'Offline';bridgeId('bridgeSap').textContent='Unavailable';setBridgeBadge(error.status?'Bridge error':'Bridge offline','error');bridgeId('bridgeDownload').hidden=Boolean(error.status);
...
const LIVE_FIELDS=['Submit Date','Task Status','ECD','Approver Team','Assigned Approver','Pending Days','Elapsed Days','Cycle Time (ECD)','Cycle Time (SOBi)'];
...
const CYCLE_WIDTHS=CYCLE_FIELDS.map(field=>({
  ...
  'Assigned Approver':130,'Pending Days':82,'Elapsed Days':82,'Cycle Time (ECD)':110,'Cycle Time (SOBi)':116,'NSR Title':190,'Task Status':144,'ECD':96,
...
const NSR_CACHE_DB='toolkitKyra',NSR_CACHE_DB_VERSION=1,NSR_CACHE_STORE='datasets',NSR_CACHE_KEY='sap-nsr-search',NSR_CACHE_SCHEMA=1;

function openNsrCache(){
  return new Promise((resolve,reject)=>{
    const api=unsafeWindow.indexedDB;if(!api){reject(new Error('IndexedDB is unavailable.'));return}
    const request=api.open(NSR_CACHE_DB,NSR_CACHE_DB_VERSION);let settled=false;
    request.onupgradeneeded=()=>{const db=request.result;if(!db.objectStoreNames.contains(NSR_CACHE_STORE))db.createObjectStore(NSR_CACHE_STORE,{keyPath:'key'})};
    request.onsuccess=()=>{if(settled){request.result.close();return}settled=true;resolve(request.result)};
    request.onerror=()=>{if(!settled){settled=true;reject(request.error||new Error('Could not open the NSR cache.'))}};
    request.onblocked=()=>{if(!settled){settled=true;reject(new Error('The NSR cache is blocked by another page.'))}};
  });
}
...
function cachedRequest(request){
  return new Promise((resolve,reject)=>{
    ...
    request.onerror=()=>reject(request.error||new Error('The NSR cache request failed.'));
  });
}

function cacheTransactionDone(transaction){
  return new Promise((resolve,reject)=>{
    transaction.oncomplete=()=>resolve();
    transaction.onerror=()=>reject(transaction.error||new Error('The NSR cache transaction failed.'));
    transaction.onabort=()=>reject(transaction.error||new Error('The NSR cache transaction was cancelled.'));
  });
}

async function readNsrCache(){
  const db=await openNsrCache();
  try{
    const entry=await cachedRequest(db.transaction(NSR_CACHE_STORE,'readonly').objectStore(NSR_CACHE_STORE).get(NSR_CACHE_KEY));
    if(!entry||entry.schema!==NSR_CACHE_SCHEMA||!Array.isArray(entry.rows)||!entry.rows.length||!validSapDate(entry.from)||!validSapDate(entry.to))return null;
    return entry;
  }finally{db.close()}
}

async function writeNsrCache(rows,range,pages){
  const db=await openNsrCache(),savedAt=Date.now();
  try{
    const transaction=db.transaction(NSR_CACHE_STORE,'readwrite');
    transaction.objectStore(NSR_CACHE_STORE).put({key:NSR_CACHE_KEY,schema:NSR_CACHE_SCHEMA,savedAt:savedAt,from:range.from,to:range.to,pages:Number(pages)||0,rows:rows});
    await cacheTransactionDone(transaction);return savedAt;
  }finally{db.close()}
}
...
function commitRaw(rawRows,meta){
  ...
  if(meta.isSap){
    const savedAt=Number(meta.savedAt),savedLabel=Number.isFinite(savedAt)&&savedAt>0?new Date(savedAt).toLocaleString():'';
    if(meta.fromCache){
      $id('heroSource').textContent=`SAP cache · ${meta.from} to ${meta.to}`;
      setSourceStatus(`Cached SAP data loaded · ${esc(meta.from)} to ${esc(meta.to)}${savedLabel?`<br>Saved ${esc(savedLabel)}`:''}`,'ok');
    }else{
      $id('heroSource').textContent=`SAP live · ${meta.from} to ${meta.to}`;
      setSourceStatus(`SAP data loaded · ${esc(meta.from)} to ${esc(meta.to)}${meta.cacheSaved?' · cached locally':''}`,'ok');
    }
  }else{
...
async function loadSapData(range=sapRange()){
  ...
  try{
    ...
    let savedAt=0,cacheError='';
    try{savedAt=await writeNsrCache(result.rows,range,result.pages)}catch(error){cacheError=clean(error&&error.message)||'Local cache unavailable';console.warn('[Toolkit Kyra] Could not update the NSR cache.',error)}
    if(run!==state.sourceRun)return;
    commitRaw(result.rows,{file:'SAP live data',isSap:true,from:range.from,to:range.to,pages:result.pages,savedAt:savedAt,cacheSaved:Boolean(savedAt)});
    if(cacheError)setSourceStatus(`SAP data loaded · ${esc(range.from)} to ${esc(range.to)}<br>${esc(cacheError)}; this page will fetch SAP again next time.`,'ok');
...
async function loadInitialNsrData(){
  const run=++state.sourceRun;if(state.sourceAbort)state.sourceAbort.abort();state.sourceAbort=null;
  setLoading(true);setSourceStatus('Checking local NSR cache…','loading');
  try{
    const cached=await readNsrCache();if(run!==state.sourceRun)return;
    if(cached){
      $id('rawFile').value='';
      commitRaw(cached.rows,{file:'SAP cached data',isSap:true,fromCache:true,from:cached.from,to:cached.to,pages:cached.pages,savedAt:cached.savedAt});
      setLoading(false);return;
    }
  }catch(error){console.warn('[Toolkit Kyra] Could not read the NSR cache; loading from SAP.',error)}
  if(run===state.sourceRun)await loadSapData(sapRange());
}
...
function parseWorkflowXml(text,statusHint=''){
  ...
  if(!entries.length)return {'Submit Date':'','Approver Team':'','Assigned Approver':'','Pending Days':'','Elapsed Days':'','Cycle Time (ECD)':'','Cycle Time (SOBi)':'',__completed:hintedTerminal==='COMPLETED',__completedAt:'',__cycleEndAt:'',__terminal:hintedTerminal,__workflow:[],__error:'No workflow data'};
...
function mergeCycle(nsr,live,details){
  ...
  row['ECD']=row.__ecd||'';
  row['Elapsed Days']=row.__terminal||row.__completed?'':elapsedDays(row['Submit Date']);
  row['Cycle Time (ECD)']=row.__ecd?elapsedDays(row['Submit Date'],row.__ecd):'';
  row['Cycle Time (SOBi)']=row.__completed&&row.__completedAt?elapsedDays(row['Submit Date'],row.__completedAt):'';
  return row;
}
...
function cycleError(err,statusHint=''){
  ...
  return {'Submit Date':'','Approver Team':'','Assigned Approver':'','Pending Days':'','Elapsed Days':'','Cycle Time (ECD)':'','Cycle Time (SOBi)':'',__completed:terminal==='COMPLETED',__completedAt:'',__cycleEndAt:'',__terminal:terminal,__error:message};
}
...
function cycleCell(row,field){
  ...
  if(['Elapsed Days','Cycle Time (ECD)','Cycle Time (SOBi)'].includes(field)&&clean(row[field])&&Array.isArray(row.__workflow)&&row.__workflow.length){
...
function cycleCellClass(row,field){
  const elapsed=clean(row['Elapsed Days']),ecdCycle=clean(row['Cycle Time (ECD)']),sobiCycle=clean(row['Cycle Time (SOBi)']);
  const elapsedOverEcd=elapsed&&ecdCycle&&Number(elapsed)>Number(ecdCycle);
  const ecdOverSobi=ecdCycle&&sobiCycle&&Number(ecdCycle)>Number(sobiCycle);
  if(field==='Elapsed Days'&&elapsedOverEcd)return 'duration-over';
  if(field==='Cycle Time (ECD)'&&(elapsedOverEcd||ecdOverSobi))return 'duration-over';
  return '';
}
